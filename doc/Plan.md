...
- [ ] *Align repository interfaces location (currently in Infrastructure; per guidance should live in Application)*
- [ ] Move repository interfaces to `FlightTracker.Application` (or formally decide & document exception)
...
- [ ] Introduce `FlightStatus` enum (replace string Status)
...
- [ ] Create `FlightTracker.Web` project (WASM)
- [ ] Integrate Radzen components (only UI layer)
- [ ] Integrate ApexCharts for stats (passport charts)
- [ ] Map component via JS interop (MapLibre or Leaflet) spike
...
- [ ] Define stats aggregation queries (total hours, routes, aircraft types placeholder)
...
- [ ] Add indexes (FlightNumber, Airport.Code) if not already in migrations
...
- [ ] README draft (project purpose, quick start)
...
1. Relocate repository interfaces to Application (or explicitly enforce current placement) + adjust DI.
2. Implement FlightService & AirportService with DTOs + mapping.
3. Introduce FlightStatus enum & replace string field.
4. Add test projects scaffold + initial Domain/Application tests.
5. Scaffold OpenSky provider interface + stub implementation.
